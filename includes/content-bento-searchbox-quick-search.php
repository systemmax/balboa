...
<form id="encore-search-form" name="encore-search-form" onsubmit="window.location.href = 'https://<?php echo $config->getPacServer() ?>/iii/encore/plus/C__S'+document.getElementById('encore-search-input').value; return false;">
    <fieldset class="bento-fieldset">
        <!-- Text input-->
        <input id="encore-search-input" name="encore-search-text" type="text" placeholder="Search Databases, Articles and <?php echo localeStrings::translate($config->getLocale(),"Catalog"); ?>" required autofocus>
        <button type="submit" id="encore-search-button" class="btn btn-primary search-button" name="encore-search-button">
            Search</button>
    </fieldset>
</form>
