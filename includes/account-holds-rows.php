...
for ($i = 0; $i < $holdsData->getTotalHolds(); ++$i) {
    ...
    if ($holdsData->getAHold($i)["recordType"] == 'i') {
        echo '<a href="#" data-toggle="tooltip" data-placement="auto" title="This is a COPY specific hold"><span class="glyphicon glyphicon-book"></span></a>  ';
    }

    // if the holdis a VOLUME specific hood, display a "book" icon indicating it.
    if ($holdsData->getAHold($i)["recordType"] == 'j') {
        echo '<a href="#" data-toggle="tooltip" data-placement="auto" title="This is a VOLUME specific hold"><span class="glyphicon glyphicon-book"></span></a>  ';
    }
...
echo '<button type="button" name="locationModal" data-target="#newLocationModal" data-backdrop="static" class="accountButton btn btn-primary disabled"><span class="glyphicon glyphicon-map-marker"></span> Change Pickup Location</button>';
