...
class carouselBibs
{
    ...
    public function __construct($config)
    {
        ...
        if (!file_exists(APP_ROOT . "store/bib/bib.dat")) {
            ...
            if ($this->config->getCarouselDataRefresh() == 0) {
                // if the refresh rate is set to 0 we never do an automatic refresh
    ...
    private function setBibIdList() {
        ...
        $apiToken = new sierraApiAccessToken($this->config);
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, html_entity_decode($this->config->getCarouselQueryString()));
    ...
    private function setCarouselBibArray($bibIdArray) {
        foreach ($bibIdArray as $thisBibId) {
            $thisBibRecord = new sierraBib($this->config,$thisBibId);
