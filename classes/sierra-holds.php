...
class sierraPatronHolds
{
    ...
    private $dateFormatString = 'M d Y'; // the US format

    public function __construct($config,$patronID)
    {
        $this->config = $config;
        ...
        switch ($this->config->getLocale()) {
            case 'CN' :
            case 'UK' :
                $this->dateFormatString = 'd M Y';
                break;
            default:
                $this->dateFormatString = 'M d Y';
        }
    }
    ...
    public function getABibIdNumber($index) {
        ...
        switch ($this->patronHolds[$index]['recordType']) {
            ...
            case "j": /* volume level hold */
                ...
                $thisItem = new sierraItem($this->config,$recordIdNo);
    ...
    public static function placeAHold($myConfig,$patronId,$bibId,$pickupLocation,$neededBy) {
    ...
    public static function changePickupLocation($myConfig,$holdId,$newLocation) {
    ...
    public static function cancelAHold($myConfig, $holdId) {
    ...
    public static function freezeAHold($myConfig, $holdId) {
    ...
    public static function unFreezeAHold($myConfig, $holdId) {
        /** @var config $config */
        $config = $myConfig;
        $uri = 'https://' . $config->getDB() . ':443/iii/sierra-api/v' . (string)$config->getApiVer() . '/patrons/holds/' . $holdId;
