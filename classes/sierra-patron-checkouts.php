...
abstract class sierraPatronCheckouts
{
    ...
    public function __construct($config,$patronID)
    {
        ...
        switch ($this->config->getLocale()) {
            case 'CN' :
            case 'UK' : $this->dateFormatString = 'd M Y';
                break;
            default:
    ...
    public function getCheckoutItemIdNumber($index) {
        ...
        $recordIdNo = substr($this->checkouts[$index]['item'],$lastSlash+1,strlen($this->checkouts[$index]['item']));
        return $recordIdNo;
    }

    public static function renewACheckout($myConfig,$ckoId) {
